...
void Orders:: makeAnOrder(const Book& book, int quantity, const Customer& whoOrdered){
...
void Orders:: removeAnOrder(int idOfOrderToRemove){
...
void Orders:: modifyAnOrder(int idOfOrderToRemove, int newQuantity){
    for(std::vector<std::shared_ptr<order>>::iterator it = doneOrders.begin(); it != doneOrders.end(); ++it)
        {
            if((*it)->orderId == idOfOrderToRemove){
